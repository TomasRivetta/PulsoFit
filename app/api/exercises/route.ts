...
type TranslationResponse = Array<Array<[string, string, unknown, unknown]>>
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    let cursor = 0;
    const translatedResults = data.map((ex): ExerciseDbExercise => {
      const translatedEx: ExerciseDbExercise = { 
        ...ex,
